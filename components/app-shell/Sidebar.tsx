...
export function Sidebar({
  ...
  const rail = (
    <div className={cn("flex h-full flex-col border-r border-zinc-300 bg-white text-zinc-600 transition-[width] duration-200", w)}>
      <div className="flex items-center gap-2 px-3 py-3.5">
        <WorkspaceSwitcher collapsed={collapsed} workspaces={workspaces} activeId={active?.id} />
      </div>
      <div className="px-3 pb-2 space-y-1">
        <button onClick={() => setPaletteOpen(true)} title={collapsed ? "Search (⌘K)" : undefined} className={cn("flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900", collapsed && "justify-center")}>
          <Search size={17} />{!collapsed && <span className="flex-1 text-left">Search</span>}{!collapsed && <kbd className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] text-zinc-500">⌘K</kbd>}
        </button>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-2">
        {NAV_GROUPS.map((g) => (
          <div key={g.title} className="mb-4">
            {!collapsed && <p className="px-2.5 pb-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-400">{g.title}</p>}
                ...
                return (
                  <Link key={it.href} href={it.href} title={collapsed ? it.label : undefined}
                    className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm transition-colors",
                      act ? "bg-indigo-600 text-white" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900",
      ...
      <div className="border-t border-zinc-200 p-3 space-y-1">
        <Link href="/settings" title={collapsed ? "Settings" : undefined}
          className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900", isActive("/settings") && "bg-zinc-100 text-zinc-900", collapsed && "justify-center")}>
      ...
      <aside className="sticky top-0 hidden h-screen shrink-0 md:block">
        <div className="relative h-full">
          ...
          <button
            ...
            className="absolute -right-3 top-5 z-20 flex h-6 w-6 items-center justify-center rounded-full border border-zinc-300 bg-white text-zinc-500 shadow-sm transition-colors hover:border-zinc-400 hover:text-zinc-900"
          >
