...
try {
  ...
  const hasInspectorHint = await page.locator("text=Select a block on the canvas").isVisible();
  ...
  for (let i = 1; i <= 12; i++) {
    await page.mouse.move(from.x + (tx - from.x) * (i / 12), from.y + (ty - from.y) * (i / 12));
  ...
  const headingText = await page.locator("h2:has-text('Your heading here')").count();
